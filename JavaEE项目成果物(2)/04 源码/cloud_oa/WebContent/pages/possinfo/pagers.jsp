...
<nav class="text-center">
	<ul class="pagination">
		...
		<c:if test="${pager.pageNum!=1 }">
			<li><a href="?pageNum=${pager.pageNum-1}${searchParam}">上页</a></li>
			<li><a href="?pageNum=1${searchParam}">1</a></li>
		</c:if>
		...
		<c:if test="${pager.pageCount<=10 }">
			...
			<c:if test="${pager.pageCount>1 }">
				...
				<c:forEach var="i" begin="2" end="${pager.pageCount }">
					...
					<c:if test="${pager.pageNum!=i }">
						<li><a href="?pageNum=${i}${searchParam}">${i}</a></li>
					</c:if>
				</c:forEach>
			</c:if>
			
			<c:if test="${pager.pageNum<pager.pageCount}">
				<li><a href="?pageNum=${pager.pageNum+1}${searchParam}">下页</a></li>
			</c:if>
		...
		<c:if test="${pager.pageCount>10 }">
			...
			<c:if test="${pager.pageNum<pager.pageCount}">
				<li><a href="?pageNum=${pager.pageCount }${searchParam}">${pager.pageCount }</a></li>
				<li><a href="?pageNum=${pager.pageNum+1}${searchParam}">下页</a></li>
			</c:if>
			<c:if test="${pager.pageNum>=pager.pageCount}">
