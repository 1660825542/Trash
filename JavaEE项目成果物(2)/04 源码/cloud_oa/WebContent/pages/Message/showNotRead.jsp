	 			...
	 			$.get("setReaded.action",{messageId:id},
						function(data){
							if(data==1){	
								getNotReadCount();							
								//成功后该条淡出
								$(key).parent().fadeOut("slow",function(){
	      	...
	      	<dropdown class="col-xs-12" style="margin-bottom:30px;"> 
				...
				<ul class="animate">
					...
					<a href="<%=request.getContextPath()%>/message/showNotRead.action">
						<li class="animate actived">未阅消息<i class="fa fa-code float-right"></i></li>
					</a>
			...
			<div class="row">        	
            	<div class="col-sm-6">
					&nbsp;
		      		...
		      		<label>全选</label>
		      		&nbsp;
		          	<input id="delete" type="button" value="设置为已读" class="btn btn-primary btn-sm" onclick="submitList('setReaded.action');">
     			</div>
     			
     			<form method="post" action="<%=request.getContextPath()%>/message/queryNotRead.action#" class="form-group">            	
			...
			<table class="table table-hover">
				...
				<c:forEach items="${list }" var="message">
					<tr style="cursor:pointer">
						...
						<td hidden name="messageId">${message.messageId}</td>
