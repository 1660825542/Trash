...
<head>
	...
	<script type="text/javascript">	
		...
		{ 
			/*单击按钮弹出模态层*/
	 		$('tr[name="notRead"]>td').not($('td').has('input')).click(function(e){
	 			var key = this;
	 			var id = $(this).parent().children('td[name="messageId"]').text();
	 			$.get("setReaded.action",{messageId:id},
						function(data){
							if(data==1){	
								getNotReadCount();							
								//成功后该条取消加粗
								$(key).parent().css("font-weight","normal");
								$(key).parent().attr("name","read");
							}else{
								layer.msg('出现错误');
							}					
						}
	 			...
	 			layer.open({
					...
					content: '<%=request.getContextPath()%>/message/showMessage.action?messageId='+id,
				}); 
				
			});
	 		/*单击按钮弹出模态层*/
	 		$('tr[name="read"]>td').not($('td').has('input')).click(function(e){
	 			var id = $(this).parent().children('td[name="messageId"]').text();
				layer.open({
					type: 2,
					area: ['700px', '450px'],
        	...
        	<dropdown class="col-xs-12" style="margin-bottom:30px;"> 
				...
				<ul class="animate">
	                ...
	                <a href="<%=request.getContextPath()%>/message/toMyMessage.action">
	                	<li class="animate actived">收件箱<i class="fa fa-cog float-right"></i></li>
	                </a>
				</ul>
			</dropdown>
		</div>            
        ...
        <div class="col-md-9">
			...
			<div class="row">        	
            	<div class="col-sm-6">
					&nbsp;
		      		...
		      		<label>全选</label>
		      		&nbsp;
		          	<input id="delete" type="button" value="删除" class="btn btn-primary btn-sm" onclick="submitList('deleteReceivedMessage.action');">
     			</div>
     			
     			<form method="post" action="<%=request.getContextPath()%>/message/queryReceivedMessage.action#" class="form-group">            	
			...
			<table class="table table-hover">
				...
				<c:choose>
					...
					<tr name="notRead" style="font-weight:bold;cursor:pointer">
						<td><input name="selected" type="checkbox"></td>						
						<td name="messageId" hidden><c:out value="${message.messageId}"/></td>
						<td hidden name="senderId"><c:out value="${message.senderId}"/></td>
						<td name="empName"></td>
					...
					<tr name="read" style="cursor:pointer" >
						<td><input name="selected" type="checkbox"></td>						
						<td name="messageId" hidden><c:out value="${message.messageId}"/></td>
						<td hidden name="senderId"><c:out value="${message.senderId}"/></td>
						<td name="empName"></td>
