...
<head>
	...
	<script type="text/javascript">
		function handle(taskId,recordId,procinsId){
			...
			$('#reimDetail').load('<%=request.getContextPath()%>/reim/queryById.action?reimId='+recordId,
					function(){
		...
		function handle2(taskId,recordId,procinsId){
			$('#recordid').val(recordId);
			$('#taskid').val(taskId);
			$('#procinsid').val(procinsId);
	  		layer.msg('打开转账接口');
                    ...
                    <c:forEach items="${list}" var="task">									
		                <tr>
    						...
    						<shiro:hasAnyRoles name="superadmin,deptadmin,manager">
		                  	...
		                  	<td>
			                	<a href="javascript:handle2(${task.taskid},${task.recordid},${task.procinsid})" class="btn btn-warning btn-xs">报销</a>     
		                  	</td>
		                  	</shiro:hasRole>
		                </tr>		                
		                </c:forEach>
        	...
        	<div class="modal-content">
                ...
                <div class="modal-footer">
                	<div id="handleOpinion">
                		<form id="handleForm" class="form-group" method="post" action="<%=request.getContextPath()%>/processreim/handleTask.action">
