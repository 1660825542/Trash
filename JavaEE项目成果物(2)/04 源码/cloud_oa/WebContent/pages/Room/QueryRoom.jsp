...
<head>
	...
	//删除确认
	function del(roomId,btnObj) {	
		
		var flag = window.confirm("确定删除?");
		
		if (flag) {
		
			var url = '<%=request.getContextPath()%>/room/delete.action?roomId=' + roomId ;
	...
	$(document).ready(function(e) {
		...
		$('#insertBtn').click(function(){			
			popWin.showWin("400","400","创建会议室","<%=request.getContextPath()%>/pages/Room/InsertRoomSmall.jsp");				
		});
	});
</script>
...
<body>
	...
	<!-- 侧边栏 -->     
		...
		<div class="col-md-7">
			...
			<table  class="table table-striped table-bordered table-condensed table-hover">
				...
				<c:forEach items="${list}" var="room">
					<tr>
						...
						<td>${room.accomNum }</td>
						<td>
						<c:if test="${room.roomState ==1}">使用中</c:if>
	    	...
	    	<shiro:hasRole name="superadmin">
	    	<input type ="button" name ="button" id="insertBtn" value="创建会议室" class="btn btn-primary">
	    	</shiro:hasRole>
			<!-- 响应式布局，移动端才会有返回主页的按钮 -->
