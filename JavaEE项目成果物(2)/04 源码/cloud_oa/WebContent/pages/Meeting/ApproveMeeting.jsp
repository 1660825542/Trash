...
<head>
	...
	/*删除确认*/
	function agree(meetId,btnObj) {	
				
		var flag = window.confirm("你确定要同意此项数据吗?");
		
		if (flag) {
		
			var url = '<%=request.getContextPath()%>/meeting/agreeState.action?meetId=' + meetId ;
			
			$.get(url,function(data){				
				$(btnObj).parent().parent().parent().parent().prev().html("通过");		
			}) ;
		}
	}
	
	function refuse(meetId,btnObj) {	
		
		var flag = window.confirm("你确定拒绝此项数据吗?");
		
		if (flag) {
		
			var url = '<%=request.getContextPath()%>/meeting/refuseState.action?meetId=' + meetId ;
			
			$.get(url,function(data){				
				$(btnObj).parent().parent().parent().parent().prev().html("拒绝");		
			}) ;
		}
	}
						...
						<td>
							<div class="btn-group btn-xs">
				                ...
				                <ul class="dropdown-menu">
				                    <li><a href="javascript:void(0)" onclick="agree(${meeting.MEET_ID},this);">同意</a></li>
				                   	<li><a href="javascript:void(0)" onclick="refuse(${meeting.MEET_ID},this);">拒绝</a></li>			                   
				                </ul>
				            </div>                  			                  
		               </td>
