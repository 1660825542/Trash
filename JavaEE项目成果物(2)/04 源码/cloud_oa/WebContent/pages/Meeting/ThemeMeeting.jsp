...
<body>
        ...
        <div class="col-md-3" id="myScrollSpy" style="margin-top:30px;">
         ...
         <!-- 侧边栏完成 -->
			...
			<form name="form1" id="form1"  action="<%=request.getContextPath() %>
				/meeting/queryByTheme.action" method="post">
				<div class="input-group">                	
	               	<input type="text" name="theme" id ="theme" placeholder="请输入会议主题" value ="${meeting.theme}" class="form-control">
	                   <span class="input-group-btn">
	                   	<input type="submit" class="btn btn-primary" value="查询会议">
	                   </span>
	                <input type="hidden"
	    	...
	    	<a href="<%=request.getContextPath() %>
						/meeting/queryByState.action" class="btn btn-large btn-block btn-primary">点击审批会议
			</a>	    	
			</shiro:hasPermission>
			<shiro:hasPermission name="apply">
	    	<a href="<%=request.getContextPath() %>
	...
	<ul class="pagination">
		...
		<c:if test="${pager.pageNum!=1}">
			<li><a href="?theme=${theme}&&pageNum=1">首页</a></li>
			<li><a href="?theme=${theme}&&pageNum=${pager.pageNum-1}">上页</a></li>
		</c:if>
		<c:if test="${pager.pageNum==1}">
		...
		<c:forEach var="i" begin="1" end="${pager.pageCount }">
			...
			<c:if test="${pager.pageNum!=i}">
				<li><a href="?theme=${theme}&&pageNum=${i}">${i}</a></li>
			</c:if>
		</c:forEach>

		<c:if test="${pager.pageNum<pager.pageCount}">
			<li><a href="?theme=${theme}&&pageNum=${pager.pageNum+1}">下页</a></li>
			<li><a href="?theme=${theme}&&pageNum=${pager.pageCount}">末页</a></li>
		</c:if>
		<c:if test="${pager.pageNum>=pager.pageCount}">
