...
<body>
	...
	<form name="form" id="form" class="form-horizontal" method="post" action="<%=request.getContextPath()%>/work/insert.action">
		<div class="form-group"> 
             ...
             <div class="col-sm-4">
             	<input type="text" value="${manager.empName }" name="managerName" id="managerName" class="form-control" readonly="readonly">               
             	<input type="hidden" name="managerId" value="${manager.empId }"> 
             </div>
              <label for="empName" class="col-sm-2 control-label">工作接受人</label>
             <div class="col-sm-4">
             	<input type="text" value="${emp.empName }" name="empName" id="empName" class="form-control" readonly="readonly">               
		...
		<div class="form-group"> 
             ...
             <div class="col-sm-4">
             	<input type="text"  datatype="*" value="<fmt:formatDate value="${startTime }" pattern="yyyy-MM-dd HH:mm:ss"/>" name="startTime" id="start" class="form-control" placeholder="请选择开始时间">               
             </div>
             <label for="empName" class="col-sm-2 control-label">结束时间</label>
             <div class="col-sm-4">
             	<input type="text" datatype="finishTime" errormsg="结束时间不合理" value="<fmt:formatDate value="${finishTime }" pattern="yyyy-MM-dd HH:mm:ss"/>" name="finishTime" id="finish" class="form-control" placeholder="请选择结束时间">               
             </div>
		</div>
		<div class="form-group"> 
             <label for="empName" class="col-sm-2 control-label">工作主题</label>
             <div class="col-sm-10">
             	<input type="text" datatype="*1-50" errormsg="最多16字" value="${taskName }" name="taskName" id="taskName" class="form-control" placeholder="请输入工作名">               
             </div>
		</div>
		<div class="form-group"> 
             <label for="empName" class="col-sm-2 control-label">工作内容</label>
             <div class="col-sm-10">
             	<textarea name="taskContent" datatype="*1-60" errormsg="最多60字" id="taskContent" class="form-control" cols="40" rows="4" datatype="*" value="${taskContent }" nulmsg="请输入工作内容"></textarea>                              
             </div>            
		</div >
		<br/>
		<div class="form-group" align="center">
			...
			<input type="hidden" name="taskDate" value="<fmt:formatDate value="<%=new java.util.Date()%>" pattern="yyyy-MM-dd"/>">
				<input type="submit" class="btn btn-primary btn-block" value="添加新任务">
			</div>
		</div>
	</form>
	</div>
